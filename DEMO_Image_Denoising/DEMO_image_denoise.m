...
%% Prompt user %%
disp(' ');
disp('***************  Low Displacement Rank Dictionary Denoising Demo  ***************');
disp('*                                                                               *');
disp('* This demo reads an image, adds random Gaussian noise, and denoises the image  *');
disp('* using a low displacement rank dictionary. The function displays the original, *');
disp('* noisy, and denoised images, and shows the resulting trained dictionary.       *');
disp('*                                                                               *');
disp('*********************************************************************************');
...
params.struct_type = 'hankel'; % Options: 'hankel' or 'toeplitz'
...
if (exp_type == 1)  % Single-run experiment
    ...
    alpha = [0:5:20 logspace(log10(25),log10(3000),45)]; % Nuclear norm regularization coefficient
end
...
save(strcat('new_SNR_sigma',num2str(sigma),'_',params.struct_type,'_',imglist(imnum).name(1:end-4),'_trainnum',num2str(params.trainnum)),'params_light','alpha','SNR','nuclear_norm_dispD','rank_dispD');
...
if (exp_type == 2)  % Complete experiment
    ...
    xlabel('Displacement Rank');
    title([imglist(imnum).name, '(input PSNR = ' num2str(10*log10(255^2/(sigma^2))) ')']);

    % Averaging results with same displacement rank 
    SNR_mean = zeros(1,params.blocksize^2);
    for kk = 1:params.blocksize^2
        if ~isempty(SNR(rank_dispD==kk))
            SNR_mean(kk) = mean(SNR(rank_dispD==kk));
        end
    end
    SNR_mean = SNR_mean(SNR_mean~=0);
    rank_dispD_mean =  unique(rank_dispD,'first');
    
    plot(rank_dispD_mean,SNR_mean,'o');
